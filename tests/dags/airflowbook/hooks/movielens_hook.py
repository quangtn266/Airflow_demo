...
class MovielensHook(BaseHook):
    ...
    def _build_session(auth=None, headers=None, retry=None):
        """
        Helper method for building a request Session.
        :param auth:  Tuple[str]
            Credentials (user, pass) to use for basic HTTP authentication.
        :param headers: Dict[str, str]
            Headers to include in every request.
        :param retry: Union[int, requests.packages.urlib3.util.retry.Retry]
    ...
    def get_ratings(self, start_date=None, end_date=None, batch_size=100):
        ...
        yield from self._get_wit_pagination(
