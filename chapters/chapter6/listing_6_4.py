...
def _wait_for_supermarket(supermarket_id_):
    ...
    sucess_files = supermarket_path / "_SUCCESS"
    return data_files and sucess_files.exists()

for supermarket_id in range(1, 5):
    wait = PythonSensor(
        task_id=f"wait_for_supermarket_{supermarket_id}",
        python_callable=_wait_for_supermarket,
        op_kwargs={"supermarket_id_": f"supermarket_id"},
        dag=dag1,
    )
...
udpate_dashboard = DummyOperator(task_id="update_dashboard", dag=dag2)
notify_new_data = DummyOperator(task_id="notify_new_data", dag=dag2)
compute_differences >> udpate_dashboard