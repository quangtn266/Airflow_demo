...
with DAG(
    ...
    def _fetch_rating(templates_dict, batch_size=1000, **_):
    ...
    fetch_ratings = PythonOperator(
        ...
        python_callable=_fetch_rating,
        templates_dict={
            "start_date": "{{ds}}",
            "end_date": "{{next_ds}}",
            "output_path": "./chapters/chapter8/data/python/ratings/{{ds}}.json",
        },
    )

    def _rank_movies(templates_dict, min_ratings=2, **_):
        intput_path = templates_dict["input_path"]
        output_path = templates_dict["output_path"]

        ratings = pd.read_json(intput_path)
    ...
    rank_movies = PythonOperator(
        ...
        templates_dict={
            "input_path": "./chapters/chapter8/data/python/ratings/{{ds}}.json",
            "output_path": "./chapters/chapter8/data/python/ratings/{{ds}}.csv"
        },
    )
