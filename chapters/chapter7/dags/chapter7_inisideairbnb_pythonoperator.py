...
from airflow.custom.postgres_to_s3_operator.py import PostgresToS3Operator
...
download_from_postgres = PostgresToS3Operator(
    ...
    query="SELECT * FROM listing WHERE download_date BETWEEN '{{ prev_ds }}' AND '{{ ds }}'",
    s3_conn_id="locals3",
...
def _crunch_numbers():
    ...
    for obj in objects:
        ...
        # There's probaly a nicer way to do this
        min_max_per_id = (
            df.groupby(["id"])
            .agg(
                download_date_min=("download_date", "min"),
                download_date_max=("donwload_date", "max"),
            )
            .reset_index()
        )
        df_with_min = (
