...
import airflow.utils.dates as airflow_dates
...
dag = DAG(
    ...
    start_date=airflow_dates.days_ago(3),
)
...
def _extract_mnist_data():
    ...
    mnist_object = s3hook.get_key(bucket_name="you-bucket", key="mnist.pkl.gz")
    mnist_object.download_fileobj(mnist_buffer)
...
sagemaker_train_model = SageMakerTrainingOperator(
    ...
    config={
        "TrainingJobName": "mnistclassifier-{{ execution_data.strftime('%Y-%m-%d-%H-%M-%S') }}",
        "AlgorithmSpecification": {
            "TrainingImage": "438346466558.dkr.ecr.eu-west-1.amazonaws.com/kmeans:1",
            "TrainingInputMode": "File",
        },
        "HyperParameter": {"k": "10", "feature_dim": "784"},
        "InputDataConfig": [
            {
                "ChannelName": "train",
                "DataSource": {
        ...
        "StoppingCondition": {"MaxRuntimeSeconds": 24 * 60 * 60},
    },
...
sagemaker_deploy_model = SageMakerEndpointOperator(
    ...
    config={
        ...
        "EndpointConfig": {
            "EndpointConfigName": "mnistclassifier-{{ execution_data.strftime('%Y-%m-%d-%H-%M-%S') }}",
            "ProductVariants": [
                {
